...
//Read initial configuration
  ...
  if(restart)
  {
    ReadConf.open("config_"+InName+".out");
    ReadVelocity.open("velocity_"+InName+".out");
...
void Move()
{
  ...
  } else // Molecular Dynamics (NVE) move
  {
    ...
    for(int i=0; i<npart; ++i){ //Verlet integration scheme
      ...
      //Some moves may be not accepted because they violate some restrinction, however this code (MD) does not perform any checks.
      accepted = accepted + 1.0;
      attempted = attempted + 1.0;
    }
  }
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Epot.open("output_epot_"+InName+".dat",ios::app);
    Ekin.open("output_ekin_"+InName+".dat",ios::app);
    Temp.open("output_temp_"+InName+".dat",ios::app);
    Etot.open("output_etot_"+InName+".dat",ios::app);
    Pres.open("output_pres_"+InName+".dat",ios::app);
...
void ConfFinal(void)
{
  ...
  WriteConf.open("config_"+InName+".out");
  WriteVelocity.open("velocity_"+InName+".out");
  for (int i=0; i<npart; ++i)
  {
